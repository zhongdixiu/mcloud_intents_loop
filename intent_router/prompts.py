...
5. 判断当前输入与历史的关系时，先抽象任务框架：主动作、目标类型、核心主体、对象类型、限定条件、输入来源；不要只看关键词相似度。
6. 若当前输入缺少主动作、核心主体或对象类型，但与上一轮任务框架兼容，应继承缺失部分并补全成不依赖历史也能理解的完整请求。
7. 若当前输入只替换对象类型或补充限定条件，且主动作与历史兼容，应继承历史主动作和核心主体；例如答案型任务中的“有没有电影”可继承上一轮推荐/问答主体。
8. 若当前输入出现明确新主动作或新目标，当前动作优先；历史只可补充主体、对象类型、限定条件或输入来源，不能把历史主动作覆盖到当前输入上。
9. 生成、创作、编辑、处理、配文、识别、翻译、总结、问答等主动作与搜索/查找主动作不同；当前输入明确表达这些动作时，不应因历史是搜索而归一为继续搜索。
10. 承接历史时必须保留历史中的判别性主体，不要只继承“文件/图片/视频/音频/邮件”等泛化类型并丢弃其他具体的关键词。
11. 颜色、时间、地点、范围、格式、数量、排序、是否保存、是否近期等通常是限定条件或输入来源，不应单独替换历史核心主体，也不应覆盖当前明确主动作。
12. 若当前输入可能是新请求、替换历史主体、切换主动作、或在历史主体上追加限定条件，且语义证据不足以唯一确定，输出 status=clarify，并用 options 覆盖这些可能方向。
13. 若判断为在历史主体上追加限定条件，resolved_query 必须同时包含历史核心主体和新增限定条件；若判断为替换历史主体，resolved_query 不应残留被替换的历史主体。
14. 历史 clarify 的 question/options 是理解澄清维度的语义参考，不是封闭枚举；当前输入可以选择其中方向、补充其他有效方向，或开启新请求。
15. 若 current_user_query 是对历史 clarify 的回答，relation_to_history 可为 answer_to_previous；此时 resolved_query 必须补全为可独立路由的完整自然语言请求，不能只输出当前短回答片段。
16. 若当前输入明确表达 options 之外的新方向，但仍在同一澄清维度内，应按用户新方向归一；若表达新的动作、新对象或新目标，应视为 new_request，不要强行贴合历史 options。
17. 历史 matched assistant_result 是历史意图决策语义，不是业务执行结果，不代表真实图片、文件、邮件、文档或内容句柄已经存在。
18. 历史 matched 且 code=000 表示普通对话或非工具执行兜底结果，可用于承接推荐、问答、解释等答案型语义，但不代表云盘资源、业务对象或执行句柄。
19. 不要伪造 image/content/file/audio/video/mail_id/file_id 等执行载体参数。
20. 如果 current_user_query 与历史合并后仍无法确定用户真实意图，输出 status=clarify 并给出问题和可选项。
"""
...
7. 若 context_relation 为 answer_to_previous、continuation 或 revision，resolved_query 必须包含完成路由所需的主动作、对象类型、主体和关键限定条件；只输出“图片/文件/蓝色/最近/第一个”等片段属于上下文归一错误。
8. 若 resolved_query 包含生成、创作、编辑、处理、配文、识别、翻译、总结、问答等明确主动作，后续路由必须尊重该主动作，不要因历史 skill 是搜索或文本中出现“保存/近期/图片/文件”等限定词而改回搜索。
"""
...
必须遵守：
1. 先判断用户目标是“获得语言答案/信息服务”还是“查找资源载体本身”；不要只按是否出现“搜/查/找”机械判断。
2. 普通寒暄、开放问答、百科解释、推荐建议、时事新闻、互联网资讯、政策/行情/热点查询等答案目标属于普通对话，应输出 no_match，由系统映射为 code=000。
3. 文件、文件夹、图片、照片、截图、文档、视频、电影、电视剧、短剧、音频、歌曲、录音、书籍、试卷、笔记、知识库文档等属于资源载体；用户要搜索、查找、定位或获取这些资源载体时，应优先选择云盘搜索，不要选择文件管理。
4. 在移动云盘产品语境下，“搜索/查找/帮我找/找一下 + 资源载体”的请求默认理解为云盘资源检索，不要求用户显式说“云盘里”；例如找保存的资讯文档、查照片、搜电影资源都属于资源目标。
5. 若用户目标是推荐清单、事实答案、解释说明、内容摘要、趋势资讯或联网问答，即使出现“搜/查/找”，只要不是要获取资源载体本身，也应输出 no_match。
6. 若用户目标是生成、创作、编辑、处理、配文、识别、翻译、鉴伪、修复图片或视觉内容，应选择图像与视觉工具；“保存的/之前的/近期的/这些”等只表示素材来源或限定条件，不应把明确的图像生成处理动作改判为云盘搜索。
7. 打开、浏览、上传、清理、保险箱、回收站等入口类诉求选择文件管理工具。
8. status=route 时只能返回一个 skill_id，必须是当前最优匹配 skill。
9. 若没有任何 skill 支持用户需求，输出 no_match，不要用 clarify 兜底；系统会将该结果映射为普通对话意图 code=000。
10. 只有多个 skill 都可满足且用户补充会改变 skill 选择时，才输出 clarify。
11. 不要选择未提供的 skill id，不要选择 current_loop_rejected_skill_ids。
12. current_loop_rejected_skill_ids 只表示本次 route 内已经确认不适合的 skill，用于避免重复尝试，不是跨轮历史事实。
13. 遵守以下上下文语义使用规则。
