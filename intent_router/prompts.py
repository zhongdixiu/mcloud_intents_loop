...
7. 若当前输入只替换对象类型、结果形态或补充限定条件，且主动作与历史兼容，应继承历史主动作和核心主体。
8. 若当前输入出现明确新主动作或新目标，当前动作优先；历史只可补充主体、对象类型、限定条件或输入来源，不能把历史主动作覆盖到当前输入上。
9. 生成、创作、编辑、处理、配文、识别、翻译、总结、问答等主动作与搜索/查找主动作不同；当前输入明确表达新主动作时，不应归一为历史主动作。
10. 承接历史时必须保留历史中的判别性主体，不要只继承“文件/图片/视频/音频/邮件”等泛化类型并丢弃其他具体的关键词。
11. 颜色、时间、地点、范围、格式、数量、排序、来源状态等通常是限定条件或输入来源，不应单独替换历史核心主体，也不应覆盖当前明确主动作。
...
7. 若 context_relation 为 answer_to_previous、continuation 或 revision，resolved_query 必须包含完成路由所需的主动作、对象类型、主体和关键限定条件；只输出孤立对象词、限定词或指代词属于上下文归一错误。
8. 若 resolved_query 包含生成、创作、编辑、处理、配文、识别、翻译、总结、问答等明确主动作，后续路由必须尊重该主动作，不要因历史 skill 类型、对象词或限定条件而改写主动作。
"""
...
必须遵守：
1. 先判断 resolved_query 的主动作、目标对象和期望结果形态，再选择 skill；不要只按关键词或历史 skill 类型机械判断。
2. 若主目标是语言答案或信息服务，包括闲聊、开放问答、百科解释、推荐建议、资讯问答、政策行情、热点查询等，且没有可执行业务 skill，应输出 no_match，由系统映射为普通对话 code=000。
...
5. 若主动作是打开、浏览、上传、清理、管理、进入入口或查看功能位置，应选择对应的管理或入口类 skill。
6. status=route 时只能返回一个 skill_id，必须是当前最优匹配 skill。
7. 若没有任何 skill 支持用户需求，输出 no_match，不要用 clarify 兜底。
8. 只有多个 skill 都可满足且用户补充会改变 skill 选择时，才输出 clarify。
9. 不要选择未提供的 skill id，不要选择 current_loop_rejected_skill_ids。
10. current_loop_rejected_skill_ids 只表示本次 route 内已经确认不适合的 skill，用于避免重复尝试，不是跨轮历史事实。
11. 遵守以下上下文语义使用规则。
