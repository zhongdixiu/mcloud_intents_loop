...
跨轮历史只注入用户输入、最终路由决策摘要、澄清问题等意图语义信息；单次 loop 内部的拒绝记录和锁定状态不跨轮复用，避免旧路径影响用户补充后的新判断。历史 matched `assistant_result` 可用于理解“它/这个/这些/确认/第二个”等表达，但不代表真实图片、文件、邮件或内容句柄已经存在；历史 no_match 只注入状态，不注入失败原因。意图识别模块不伪造执行载体参数。
